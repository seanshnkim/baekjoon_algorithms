...
for i in range(N):
    for j in range(N):
        ...
        if j < N - 1 and arr_2D[i][j] != arr_2D[i][j+1]:
            ...
            # 열에 대해 cnt 계산:
            #FIXME: swap하지 않았을 때의 max 값을 고려해주지 않아 반례가 생김
            # 즉, max_cols[j+1]인 경우도 고려해야 함
            max_cnt_col = max(max_cols[j], max_cols[j+1], max(search_col(j), search_col(j+1)))
        ...
        if i < N -1 and arr_2D[i][j] != arr_2D[i+1][j]:
            ...
            # 행에 대해 cnt 계산:
            #FIXME: 마찬가지로 swap하지 않았을 때의 max 값을 고려해야 함
            max_cnt_row = max(max_rows[i], max_rows[i+1], max(search_row(i), search_row(i+1)))
